...
def save_model_and_metadata(model, X, y, config, model_name=None):
    """保存训练好的模型和相关元数据。"""
...
def main():
    ...
    # 保存特征重要性图表
    feature_importance_path = os.path.join(CONFIG["data"]["model_output_path"], "feature_importance.png")
    plot_feature_importance(model, X, save_path=feature_importance_path)
    
    # 保存模型和元数据
    model_name, model_path = save_model_and_metadata(model, X, y, CONFIG)
    
    print(f"\n=== 训练完成 ===")
    print(f"模型名称: {model_name}")
    print(f"模型文件: {model_path}")
    print("模型已保存为PKL格式，可供测试区域和FastAPI区域调用。")
